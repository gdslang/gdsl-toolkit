...
static void simulator_variable_write(struct context *context, struct rreil_variable *variable, struct data data) {
  if(data.bit_length) simulator_register_write(context, variable->id, data, variable->offset);
}

static void simulator_variable_limited_write(struct context *context, struct rreil_variable_limited *varl,
    struct data data) {
  if(data.bit_length) simulator_register_write(context, varl->id, data, varl->offset);
}
...
static void simulator_variable_limited_read(struct context *context, struct rreil_variable_limited *varl,
    struct data data) {
...
static void simulator_branch_simulate(struct context *context, struct rreil_address *target) {
  ...
  for(size_t j = 0; j < target->size / 8; ++j)
    if(address.defined[j] != 0xff) defined = 0;
  ...
  if(defined) {
    ...
    if(!equal) context->memory.jump(context->memory.closure, address.data, target->size);
  }
...
static enum simulator_error simulator_prim_simulate(struct context *context, char *op,
    struct rreil_variable_limited_tuple *lhs, struct rreil_variable_limited_tuple *rhs) {
//	if(!strcmp(op, "mods")) {
...
static enum simulator_error simulator_statement_simulate(struct context *context, struct rreil_statement *statement) {
  ...
  switch(statement->type) {
    ...
    case RREIL_STATEMENT_TYPE_ITE: {
      ...
      if(!(data.defined[0] & 0x01) || (data.data[0] & 0x01)) error |= simulator_statements_simulate(context,
          statement->ite.then_branch);
      else error |= simulator_statements_simulate(context, statement->ite.else_branch);
    ...
    case RREIL_STATEMENT_TYPE_CBRANCH: {
      ...
      if(!(data.defined[0] & 0x01) || (data.data[0] & 0x01)) simulator_branch_simulate(context,
          statement->cbranch.target_true);
      else simulator_branch_simulate(context, statement->cbranch.target_false);
      context_data_clear(&data);
      break;
    }
