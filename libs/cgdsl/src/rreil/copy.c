...
struct rreil_id *rreil_id_copy(struct rreil_id *id) {
  ...
  if(id->type == RREIL_ID_TYPE_ARCH) {
    id_copy->type = RREIL_ID_TYPE_ARCH;
    id->arch = (char*)malloc(strlen(id->arch) + 1);
    strcpy(id_copy->arch, id->arch);
  } else
#endif
  *id_copy = *id;
...
struct rreil_exception *rreil_exception_copy(struct rreil_exception *exception) {
  ...
  if(exception->type == RREIL_EXCEPTION_TYPE_ARCH) {
    exception_copy->type = RREIL_EXCEPTION_TYPE_ARCH;
    exception_copy->arch = (char*)malloc(strlen(exception->arch) + 1);
    strcpy(exception_copy->arch, exception->arch);
  } else
#endif
