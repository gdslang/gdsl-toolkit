...
static obj_t sem_address(state_t state, int_t size, obj_t address) {
  struct rreil_address *address_ = (struct rreil_address*)malloc(sizeof(struct rreil_address));
  address_->size = size;
...
static obj_t sem_cmpeq(state_t state, obj_t opnd1, obj_t opnd2) {
  struct rreil_comparator *comparator = (struct rreil_comparator*)malloc(sizeof(struct rreil_comparator));
...
static obj_t sem_varl(state_t state, obj_t id, int_t offset, int_t size) {
  struct rreil_variable_limited *variable = (struct rreil_variable_limited*)malloc(
...
static obj_t sem_varls_next(state_t state, obj_t next, obj_t list) {
  ...
  if(variable_tuple->variables_length >= variable_tuple->variables_size) {
    ...
    variable_tuple->variables = (struct rreil_variable_limited**)malloc(
        sizeof(struct rreil_variable_limited*) * variable_tuple->variables_size);
  }
...
static obj_t sem_stmts_next(state_t state, obj_t next, obj_t list) {
  ...
  if(statements->statements_length + 1 > statements->statements_size) {
    statements->statements_size = statements->statements_size ? statements->statements_size << 1 : 4;
    statements->statements = (struct rreil_statement**)realloc(statements->statements,
