...
val asm-convert-insn cbs insn = cbs.insn (insn.length + 0) insn.mnemonic (asm-convert-annotations cbs insn.annotations) (asm-convert-opnds cbs insn.opnds)
...
val asm-convert-opnd cbs opnd = case opnd of
 ...
 | ASM_SCALE s: cbs.opnd.scale (s.factor + 0) (asm-convert-opnd cbs s.rhs)
...
val asm-convert-boundary cbs b = case b of
   ASM_BOUNDARY_SZ sz: cbs.boundary.sz (sz + 0)
 | ASM_BOUNDARY_SZ_O szo: cbs.boundary.sz_o (szo.size + 0) (szo.offset + 0)
end
