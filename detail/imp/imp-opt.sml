      ...
      let
         ...
         fun lub (VARstype x, VARstype y) =
            let
               ...
               val yRoot = find (tt,y)
               val _ = iter := (!iter) + 1
               val _ = if !iter > 100 then (
                        TextIO.print ("non-termination for lub of " ^ showSType t1 ^ " and " ^ showSType t2 ^ ", that is, of " ^showSType (inlineSType s t1) ^ " and " ^ showSType (inlineSType s t2) ^ "\n");
                        raise TypeOptBug
                     ) else ()
