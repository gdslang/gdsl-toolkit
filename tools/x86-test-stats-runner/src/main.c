...
static void for_name(char *name) {
  ...
  if(!e->data) e->data = (struct insn_data*)calloc(sizeof(struct insn_data), 1);
  else free(copy);
  insn_data = (struct insn_data*)e->data;
}

static void incf(struct tester_result *result, struct feature_data *features, size_t i) {
  if(result->type == TESTER_RTYPE_SUCCESS) features[i].success++;
  else features[i].failed++;
}
...
static char test_instruction(struct feature_data *features, struct hash_array *insn_types_, uint8_t *data,
  ...
  if(result.type == TESTER_RTYPE_DECODING_ERROR) return 1;
  ...
  if(insn_data) {
    ...
    switch(result.type) {
      ...
      case TESTER_RTYPE_EXECUTION_ERROR: {
        ...
        if(result.execution_result.type == EXECUTION_RTYPE_SIGNAL) switch(result.execution_result.signum) {
          case SIGSEGV: {
...
//		if(result.type != TESTER_RTYPE_DECODING_ERROR) { //Always true ;-)
    ...
    for(size_t i = 1; i < X86_FEATURES_COUNT; ++i)
      if(result.features & (1 << (i - 1))) incf(&result, features, i);
    if(!result.features) incf(&result, features, 0);
//		}
  }
